...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 2. Load Auth state without auto-kick
    auth.onAuthStateChanged(async (user) => {
        currentUser = user;
        // Fetch latest provider data from Firestore to get real-time isOnline status
        const providerId = providerData.uid || providerData.id;
        const freshSnap = await getDoc(doc(db, "users", providerId));
        if (freshSnap.exists()) {
            providerData = { ...providerData, ...freshSnap.data() };
        }
    ...
    document.getElementById("bookSessionBtn")?.addEventListener("click", () => {
        if (!currentUser) return handleGuestBooking();
...
// ── GUEST REDIRECT ALERT ──────────────────────────────
function handleGuestBooking() {
    sessionStorage.setItem('redirectAfterLogin', window.location.href);
    
    // Luxurious alert
    const sb = document.createElement("div");
    sb.style.cssText = "position:fixed; bottom:30px; left:50%; transform:translateX(-50%); background:var(--green-dark, #0d3d2e); color:#fff; padding:14px 28px; border-radius:50px; font-weight:600; box-shadow:0 10px 25px rgba(0,0,0,0.2); z-index:9999; animation:fadeUp 0.4s ease; display:flex; align-items:center; gap:8px;";
    sb.innerHTML = `<span class="material-symbols-outlined" style="font-size:1.2rem;">lock</span> Please log in to book a session.`;
...
function populateProfileUI() {
    ...
    if (d.isOnline === false) {
        ...
        if (bookBtn) {
            ...
            bookBtn.innerHTML = '<span class="material-symbols-outlined">cloud_off</span> Provider Offline';
        }
        if (monthlyBtn) {
        ...
        statusNotice.style.cssText = "background:#FEF2F2; color:#DC2626; padding:12px; border-radius:12px; margin-bottom:20px; font-size:14px; font-weight:600; text-align:center; border:1px solid #FCA5A5;";
        statusNotice.innerHTML = `<span class="material-symbols-outlined" style="vertical-align:middle; font-size:18px;">error</span> This provider is currently offline and not accepting new bookings.`;
        document.getElementById("profileMain")?.prepend(statusNotice);
    }
}
...
function renderDateRow() {
    ...
    upcomingDates.forEach((date, idx) => {
        // Short name for UI chip (e.g., "Mon")
        const dayNameShort = date.toLocaleDateString("en-US", { weekday: "short" });
        // Full name to match scheduleMap keys (e.g., "Monday")
        ...
        // Provider saves schedule using full day names, and sets "Off" if closed.
        const isAvailable = scheduleMap[dayNameLong] && scheduleMap[dayNameLong] !== "Off";
        ...
        chip.addEventListener("click", () => onDateChipClick(date, dayNameLong, isAvailable, chip));
...
async function submitBooking(isMonthly) {
    ...
    if (providerData.isOnline === false) {
        alert("This provider is currently offline and cannot accept bookings.");
        return;
    }
    if (isBooking) return;
    isBooking = true;
